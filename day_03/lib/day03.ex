defmodule Day03 do
  ...
  def lowest_steps_to_intersection(wire1, wire2) do
    route1 = Wire.build(wire1)
    route2 = Wire.build(wire2)

    intersections = route1 -- route1 -- route2
    IO.puts("Intersections:")
    IO.inspect(intersections)

    route1_points =
      route1
      |> Enum.with_index(1)
      |> Enum.filter(fn {point, _index} -> Enum.member?(intersections, point) end)
      |> Enum.into(%{})
    ...
    IO.puts("route 1 points")
    IO.inspect(route1_points)
    IO.puts("route 2 points")
    IO.inspect(route2_points)

    intersections
    |> Enum.map(fn point -> Map.get(route1_points, point) + Map.get(route2_points, point) end)
    |> Enum.min
  end
